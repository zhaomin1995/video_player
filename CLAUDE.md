...
- **Sleep prevention** — `Utilities/SleepPreventer.swift` is a singleton calling `ProcessInfo.beginActivity(options:)`. Two activity flavors: video (`idleDisplaySleepDisabled | idleSystemSleepDisabled`) vs audio (`idleSystemSleepDisabled` only — display sleeps after timeout so a 2-hr podcast doesn't burn battery on a lit screen). `PlayerViewController.updateSleepAssertion(isPlaying:)` is called from BOTH engine `didUpdateStatus` callbacks; engages on `isPlaying=true && Defaults.preventSleepWhilePlaying`, releases otherwise. `hasVideo` derived from active engine's `videoSize` — audio-only files get the system-only activity. Verify with `pmset -g assertions | grep -A2 "Awesome Player"`. Don't replace with `IOPMAssertionCreateWithName` — the old IOKit path is what IINA migrated AWAY from (see their SleepPreventer comments) because beginActivity gives macOS more leeway to make scheduling decisions
- **Mini Player / Music Mode** — `UI/Window/MiniPlayerWindowController.swift` is an NSPanel (hudWindow + nonactivatingPanel + floating level) that mirrors state from the active engine via a 0.5s polling timer (cheap, no observer wiring needed). Doesn't embed video — moving libvlc's `renderView` between window hierarchies mid-playback is fragile because the layer is bound at `libvlc_media_player_set_nsobject` time. Audio-mode-only is the canonical music-mode UX; video users can use PiP. Toggle via `AppDelegate.toggleMiniPlayer(_:)` (Window menu ⌘⇧M) — main window orderOut while mini is up; mini close routes back through the toggle so the user always lands on a visible window
- **Interactive crop** — `UI/Player/CropOverlayView.swift` is an overlay added on top of `videoView` while in crop mode. The drag rect is in our view's points (origin bottom-left); `cropGeometry()` accounts for letterbox (the actually-displayed video may not fill our bounds — picks `srcAspect > viewAspect` for letterbox vs pillarbox) and maps to libvlc's source-pixel "WxH+X+Y" format with top-left origin. Wiring: `PlayerViewController.beginInteractiveCrop()` creates the overlay, sets first responder for Escape-to-cancel, and `vlcEngine?.setCropGeometry(g)` on apply. AVPlayer path crop not yet wired — would need `AVPlayerLayer.contentsRect` (CGRect with normalized 0–1 coords); easy to add when needed
- **Video Filters chain** — `UI/Player/VideoFiltersPanelController.swift` (singleton) writes to UserDefaults; `VLCPlayerEngine.open()` reads via `VideoFiltersPanelController.buildFilterChainOption()` and passes as `:video-filter=…` media option. libvlc 3.x CANNOT add/remove video filters at runtime — the chain is set at media-creation time only. So the panel surfaces a "Reopen Current File" button that re-runs `PlayerViewController.openFile(url:)`. 5 filters supported: sharpen + grain (parameterized with sigma/variance sliders), posterize + invert + mirror (toggles). Adding more is one entry in `buildFilterChainOption()` + one default key + one toggle in the panel — but skip wave/ripple/psychedelic, they're broken or unstable on macOS in libvlc 3.x
- **URL scheme + Services** — Info.plist's `CFBundleURLTypes` registers `awesomeplayer://`; AppKit routes invocations into `AppDelegate.application(_:open:)`. `routeIncomingURL(_:)` branches on file:// / http(s) / bare-path. HTTP(S) URLs go through `URLOpenCoordinator.openExternalURL(_:)` (newly added, skips the input dialog and runs the same isDirectMediaURL/yt-dlp resolution as `begin()`). The Services menu entry in Info.plist's `NSServices` calls `handleServicesURL:userData:error:` which reads from the pasteboard. Bookmarklet at `Scripts/safari-bookmarklet.html` is the cross-browser way to fire the scheme — drag-to-bookmarks-bar install, no extension target needed. NOT a Safari App Extension — that requires a separate code-signed extension target with its own bundle ID, and ad-hoc signing won't let it load. URL scheme + Services + bookmarklet is the same UX outcome with zero extra distribution surface
- **CLI** — `Scripts/awesomeplayer` is a bash wrapper around `open -a "Awesome Player"` (path resolution via `python3 -c "import os; os.path.abspath(...)"` since macOS doesn't ship `realpath`). `Scripts/install-cli.sh` symlinks it into `/usr/local/bin` (or a custom `--prefix`); needs sudo for /usr/local. Not a Swift target — keeping it shell avoids a second binary in the .app and the codesign/build complexity. URL scheme handling means the CLI can pass `awesomeplayer://` URLs too: `open -a "Awesome Player" "awesomeplayer://https%3A//..."`
